...
interface StoreTemplate {
  ...
  sample_products?: Array<{ name: string; price: number; description?: string; image?: string; category?: string }>;
  sample_services?: Array<{ name: string; price: number; duration: number; description?: string }>;
}
  ...
  const handleApply = async (tmpl: StoreTemplate) => {
    ...
    try {
      ...
      if (!nextSlug) {
        ...
        const check = await validateSlug(candidate, user.id);
        nextSlug = check.ok ? check.slug : `${candidate || 'loja'}-${user.id.slice(0, 4)}`;
      }

      await supabase
        .from('profiles')
        .update({
          store_mode: tmpl.config.storeMode,
          store_description: tmpl.description,
          template_id: tmpl.id,
      ...
      // Popula amostras
      const samplesP = tmpl.sample_products || [];
      const samplesS = tmpl.sample_services || [];

      if (samplesP.length > 0) {
      ...
      <main className="flex-1 mt-20 p-6 pb-32">
          ...
          {templates.map((tmpl) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                {selectedId === tmpl.id && <CheckCircle className="w-5 h-5 text-indigo-500" />}
              </div>
        ...
        {templates.length === 0 && (
          <div className="text-center py-20 bg-white/5 border border-dashed border-white/5 rounded-[2rem]">
            ...
            <p className="text-slate-500 font-bold text-sm">Nenhum modelo disponível</p>
          </div>
        )}
      </main>
        ...
        {previewTemplate && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="flex-1 overflow-y-auto p-8 text-center">
              <div className="w-24 h-24 bg-white/5 rounded-3xl mx-auto mb-6 flex items-center justify-center border border-white/10">
                {previewTemplate.config.storeMode === 'scheduling' ? <Calendar className="w-10 h-10 text-emerald-400" /> :
                 previewTemplate.config.storeMode === 'store+ai' ? <MessageCircle className="w-10 h-10 text-fuchsia-400" /> :
              ...
              <div className="space-y-4">
                {[
                  ...
                  'Gestão de Catálogo Integrada',
                  'Subdomínio @arroba.live',
                ].map((label) => (
            ...
            <div className="p-6 bg-slate-900/80 backdrop-blur-xl border-t border-white/5 flex gap-3">
              ...
              <button onClick={() => handleApply(previewTemplate)} disabled={applying}
