  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col pb-32">
      <header className="fixed top-0 w-full z-50 bg-slate-900/40 backdrop-blur-2xl border-b border-white/5 flex justify-between items-center px-4 h-16 shadow-sm">
        <button onClick={() => setScreen('business-profile')} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/5 transition-colors active:scale-90">
          <ArrowLeft className="w-6 h-6 text-slate-400" />
        </button>
        <div className="flex flex-col items-center">
          <h1 className="font-extrabold text-white tracking-tighter">Arroba</h1>
          <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Catalog</span>
        </div>
        <div className="flex items-center gap-1">
          <button className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/5"><Search className="w-5 h-5 text-slate-400" /></button>
          <button className="relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/5 active:scale-90">
        ...
        <div className="flex gap-2 px-4 overflow-x-auto no-scrollbar mb-6">
          {['All Styles', "Summer '24", 'Accessories', 'Premium'].map((filter, i) => (
            <button key={filter} className={`
              ...
              ${i === 0 ? 'bg-primary text-white shadow-primary-glow scale-105' : 'bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10'}
            `}>
              {filter}
            </button>
          ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            {products.map((prod) => (
              <div key={prod.id} className="group flex flex-col">
                <div className="relative aspect-[3/4] rounded-[2rem] overflow-hidden mb-3 shadow-neumorphic-lift bg-slate-900/50 cursor-pointer active:scale-95 transition-all border border-white/5">
                  ...
                  {prod.tag && (
                    <div className={`
                      ...
                      ${prod.tag.includes('%') ? 'bg-red-500 text-white' : 'bg-indigo-500/90 text-white'}
                    `}>
                ...
                <div className="px-2">
                  <h3 className="font-bold text-sm truncate text-white leading-tight">{prod.name}</h3>
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-wide mb-2">{prod.brand}</p>
                  <div className="flex justify-between items-center">
                    <div className="flex flex-col">
                      <span className={`font-black text-lg ${prod.originalPrice ? 'text-red-400' : 'text-white'}`}>
                        R$ {prod.price.toFixed(2).replace('.', ',')}
                      </span>
                      {prod.originalPrice && (
                        <span className="text-[10px] text-slate-500 line-through font-bold">R$ {prod.originalPrice.toFixed(2).replace('.', ',')}</span>
                      )}
                    </div>
                    <button className="w-10 h-10 rounded-full bg-indigo-500 text-white flex items-center justify-center shadow-primary-glow active:scale-90 transition-transform">
